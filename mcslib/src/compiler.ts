...
export type Export<Args extends McsType[]> = {
  name: string,
...
export class Compiler {
  ...
  async export<const Args extends McsType[]>(
    {
      name,
      fn,
      args,
    ...
    try {
      ...
      if (length > 0) {
        ...
        for (let i = 0; i < length; i++) {
          ...
          if (type !== McsEmpty) {
            keys.push(`a${i}:${serialize(type, `$(${name})`)}`);
          }
        }
  ...
  >(f: McsFunction<Sig>): Promise<CompileResult> {
    ...
    const id = `mcslib/fn${this.compileMap.size}`;
